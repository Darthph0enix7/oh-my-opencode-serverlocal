...
1. Define named presets in `oh-my-opencode-serverlocal.jsonc` under the `presets` field
...
| Method | How | Persists? |
|--------|-----|-----------|
| Config file | Set `"preset": "cheap"` in `oh-my-opencode-serverlocal.jsonc` | Yes, across restarts |
