...
Centralizes configuration loading, validation, schema definitions, and runtime state management for the oh-my-opencode-serverlocal plugin. This folder implements the configuration pipeline that merges user preferences, project overrides, and preset-based agent configurations, providing validated runtime configuration objects to the rest of the plugin.
...
```
1. Discovery Phase
   ├─ User config: $OPENCODE_CONFIG_DIR/oh-my-opencode-serverlocal.{jsonc,json}
   ├─ Project config: <directory>/.opencode/oh-my-opencode-serverlocal.{jsonc,json}
