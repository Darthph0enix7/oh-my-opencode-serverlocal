...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-opencode-serverlocal', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-opencode-serverlocal] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  try {
    ...
    console.error(
      `[oh-my-opencode-serverlocal] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
