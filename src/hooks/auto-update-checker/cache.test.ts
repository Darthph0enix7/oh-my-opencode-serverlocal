  ...
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest/node_modules/oh-my-opencode-serverlocal/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest/package.json',
      });
  ...
  describe('preparePackageUpdate', () => {
    ...
    test('updates packages wrapper dependency and removes installed package', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/oh-my-opencode-serverlocal@latest/node_modules/oh-my-opencode-serverlocal',
      );
        ...
        (p: string) => {
          if (
            ...
            return JSON.stringify({
              dependencies: {
                'oh-my-opencode-serverlocal': '0.9.1',
              },
            });
          }
          return '';
      ...
      expect(JSON.parse(writtenData[0])).toEqual({
        dependencies: {
          'oh-my-opencode-serverlocal': '0.9.11',
        },
      });
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/oh-my-opencode-serverlocal'),
      );
      const readSpy = spyOn(fs, 'readFileSync').mockReturnValue(
        JSON.stringify({
          dependencies: {
            'oh-my-opencode-serverlocal': '1.0.1',
          },
        }),
      );
      const writeSpy = spyOn(fs, 'writeFileSync').mockImplementation(() => {});
      ...
      const result = preparePackageUpdate('1.0.1', 'oh-my-opencode-serverlocal', null);
