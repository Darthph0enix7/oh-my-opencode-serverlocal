...
Centralized utilities and shared abstractions used across the oh-my-opencode-serverlocal plugin. This folder provides:
...
- **Logger** (`logger.ts`): File-based logging with 7-day retention, automatic directory creation, and write queuing. Logs are written to `~/.local/share/opencode/log/oh-my-opencode-serverlocal.<sessionId>.log` and cleaned up on initialization.
...
2. Logs are appended to `~/.local/share/opencode/log/oh-my-opencode-serverlocal.<sessionId>.log`
